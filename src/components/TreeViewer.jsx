...
function findPathToNode(tree, htsno, path = []) {
  for (const node of tree) {
    if (!node) continue;
    if (node.htsno === htsno) return [...path, node];
    if (node.children && Array.isArray(node.children)) {
      const found = findPathToNode(node.children, htsno, [...path, node]);
...
8. Output only JSON array of top 10 like:
  ...
  return (
    <div className="p-6 h-screen flex flex-col">
      ...
      {searchError && <div className="error-message mb-4">{searchError}</div>}
